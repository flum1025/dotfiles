...
usage() {
  ...
  echo "  -a <name>           AWS profile name" >&2
  echo "  -g <name>           AWS auto scalling group name" >&2
}
...
while [ $# -ne 0 ]; do
  case $1 in
    ...
    -g)
      ...
      hosts=$(aws ec2 describe-instances --profile $aws_profile_name --output text --query "Reservations[*].Instances[*].PublicIpAddress" --filter "Name=tag:aws:autoscaling:groupName,Values=$1"); shift
      ;;
    -h)
      usage
      exit 0
